...
On clicking "*Update Cart*" button, it updates the item count (of product) in the cart (in CoreData) and dismisses the screen.
...
At botton, it has a label and a button. The label displays the total items count in the cart and value of items in the cart. The "*Checkout*" button can be used to check out current cart. Clicking it would display a alert with confirmation, also resets everything in the cart. Dismissing alert would take you back to Product List screen.
...
Feel free to get in touch with at Bhargavi[.dev](https://jogendra.dev/)
